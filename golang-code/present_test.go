...
func BenchmarkBlock_Encrypt(b *testing.B) {
	for _, c := range cases {
		b.Run(fmt.Sprintf("%d-bit key", len(c.Key)*4), func(b *testing.B) {
			key := decodeHex(c.Key)
			cipher, err := present.NewCipher(key)
			if err != nil {
				b.Fatal(err)
			...
			ciphertext := decodeHex(c.Ciphertext)
			assert.Equal(b, ciphertext, dst)
		})
	}
}

func BenchmarkBlock_Decrypt(b *testing.B) {
	for _, c := range cases {
		b.Run(fmt.Sprintf("%d-bit key", len(c.Key)*4), func(b *testing.B) {
			key := decodeHex(c.Key)
			cipher, err := present.NewCipher(key)
